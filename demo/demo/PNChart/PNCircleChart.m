  ...
  overrideLineWidth:(NSNumber *)overrideLineWidth
{
    ...
    if (self) {
        ...
        UIBezierPath *circlePath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.frame.size.width/2.0f, self.frame.size.height/2.0f)
                                                                  radius:(self.frame.size.height * 0.4) - ([_lineWidth floatValue]/2.0f)
        ...
        [_countingLabel setFont:[UIFont boldSystemFontOfSize:20.0f]];
        [_countingLabel setTextColor:[UIColor grayColor]];
        [_countingLabel setBackgroundColor:[UIColor clearColor]];
        [_countingLabel setCenter:CGPointMake(self.frame.size.width/2.0f, self.frame.size.height/2.0f - 10)];
        _countingLabel.method = UILabelCountingMethodEaseInOut;
        
        
        //"相似"label
        CGFloat resembleCenterX = self.countingLabel.center.x;
        CGFloat resembleCenterY = self.frame.size.height/2.0f + 10;
        UILabel *resembleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 100.0, 50.0)];
        [resembleLabel setCenter:CGPointMake(resembleCenterX, resembleCenterY)];
        [resembleLabel setTextAlignment:NSTextAlignmentCenter];
        [resembleLabel setFont:[UIFont boldSystemFontOfSize:16.0f]];
        [resembleLabel setTextColor:[UIColor grayColor]];
        [resembleLabel setBackgroundColor:[UIColor clearColor]];
        resembleLabel.text = [NSString stringWithFormat:@"相似"];
        
        
        if (_displayCountingLabel) {
            [self addSubview:_countingLabel];
            [self addSubview:resembleLabel];
        }
